    ...
    <section className="pt-4">
      <div className="container">
        <div className="row rounded-3 my-3 gy-3">
          {data?.data.data.filter((product) => product.brand._id == id).length === 0 ? (
            
            <div className="d-flex align-items-center justify-content-center text-center flex-column">
            <img  src={imgone} alt=""/>
             <h1 className="fw-bold text-center">No products available for this brand.<Link to="/brands" className="link-info">back to brand</Link></h1></div>
          ) : (
            data?.data.data
